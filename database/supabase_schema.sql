...
-- ─────────────────────────────────────────────────────────────
-- 0.1 FUNÇÃO updated_at automático
-- ─────────────────────────────────────────────────────────────
...
-- Trigger: cria perfil automático quando usuário se cadastra
...
-- ─────────────────────────────────────────────────────────────
-- 3. PERFIL CORPORATIVA (comprador empresa)
-- ─────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS public.perfis_transportadora (
  ...
  nome_fantasia       TEXT,
  cnpj                TEXT        NOT NULL,
  rntrc               TEXT,                     -- Registro ANTT
  vehicle_types       TEXT[],                   -- ['carreta_boiadeira','bau_climatizado',...]
  max_capacity_kg     NUMERIC(10,2),
  live_transport      BOOLEAN     NOT NULL DEFAULT TRUE,
  city                TEXT        NOT NULL,
  state               CHAR(2)     NOT NULL,
  description         TEXT,
  score               NUMERIC(4,2),             -- score médio calculado
  created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Regiões de atendimento da transportadora
...
CREATE INDEX idx_anuncios_seller      ON public.anuncios(seller_id);
CREATE INDEX idx_anuncios_category    ON public.anuncios(category);
CREATE INDEX idx_anuncios_state       ON public.anuncios(state);
CREATE INDEX idx_anuncios_status      ON public.anuncios(status);
CREATE INDEX idx_anuncios_price       ON public.anuncios(price_total);
CREATE INDEX idx_anuncios_created     ON public.anuncios(created_at DESC);
CREATE INDEX idx_anuncios_search      ON public.anuncios USING GIN(search_vector);
CREATE INDEX idx_anuncios_trgm_title  ON public.anuncios USING GIN(title gin_trgm_ops);
CREATE INDEX idx_anuncios_trgm_breed  ON public.anuncios USING GIN(breed gin_trgm_ops);

-- Trigger: atualiza search_vector ao criar/atualizar anúncio
...
CREATE INDEX idx_fotos_anuncio ON public.fotos_anuncio(anuncio_id);
...
-- Trigger: recalcula traceability_score no anúncio quando rastreabilidade muda
CREATE OR REPLACE FUNCTION recalculate_traceability()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
  doc_count  INTEGER;
  total_docs INTEGER := 8;
BEGIN
  SELECT (
  ...
  UPDATE public.anuncios
    SET traceability_score = ROUND((doc_count::NUMERIC / total_docs) * 100)
...
CREATE TRIGGER trg_propostas_updated_at BEFORE UPDATE ON public.propostas
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE INDEX idx_propostas_anuncio ON public.propostas(anuncio_id);
CREATE INDEX idx_propostas_buyer   ON public.propostas(buyer_id);
CREATE INDEX idx_propostas_seller  ON public.propostas(seller_id);
CREATE INDEX idx_propostas_status  ON public.propostas(status);

-- Trigger: incrementa proposals_count no anúncio
...
CREATE TABLE IF NOT EXISTS public.contratos (
  id            UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  proposta_id   UUID        NOT NULL UNIQUE REFERENCES public.propostas(id),
  anuncio_id    UUID        NOT NULL REFERENCES public.anuncios(id),
  buyer_id      UUID        NOT NULL REFERENCES public.profiles(id),
  seller_id     UUID        NOT NULL REFERENCES public.profiles(id),
...
CREATE INDEX idx_cotacoes_solicitacao ON public.cotacoes_frete(solicitacao_id);
CREATE INDEX idx_cotacoes_carrier     ON public.cotacoes_frete(carrier_id);

-- ─────────────────────────────────────────────────────────────
-- 13. MENSAGENS (chat por negociação)
-- ─────────────────────────────────────────────────────────────
...
CREATE INDEX idx_mensagens_anuncio  ON public.mensagens(anuncio_id);
CREATE INDEX idx_mensagens_sender   ON public.mensagens(sender_id);
CREATE INDEX idx_mensagens_receiver ON public.mensagens(receiver_id);
...
CREATE TABLE IF NOT EXISTS public.notificacoes (
  id         UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id    UUID        NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  type       TEXT        NOT NULL,   -- 'nova_proposta','proposta_aceita','nova_mensagem','frete_cotado',...
  title      TEXT        NOT NULL,
  body       TEXT,
  payload    JSONB,
  read       BOOLEAN     NOT NULL DEFAULT FALSE,
...
CREATE INDEX idx_notificacoes_user ON public.notificacoes(user_id, read, created_at DESC);

-- ─────────────────────────────────────────────────────────────
-- 15. FUNÇÃO: busca full-text de anúncios
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 16. FUNÇÃO: incrementar visualizações
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- 17. FUNÇÃO: criar notificação automática ao aceitar proposta
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  IF OLD.status = 'aguardando' AND NEW.status = 'aceita' THEN
    ...
    VALUES (
      ...
      'O vendedor aceitou sua proposta. Acesse para assinar o contrato.',
  ...
  IF OLD.status = 'aguardando' AND NEW.status = 'recusada' THEN
    ...
    VALUES (
      ...
      'proposta_recusada',
      'Proposta não aceita',
      'O vendedor não aceitou sua proposta desta vez.',
      jsonb_build_object('proposta_id', NEW.id, 'anuncio_id', NEW.anuncio_id)
    );
  END IF;
  RETURN NEW;
...
-- Notificação ao vendedor quando proposta é enviada
...
-- perfis_produtor
CREATE POLICY "produtor: leitura pública"
  ON public.perfis_produtor FOR SELECT USING (true);
CREATE POLICY "produtor: owner all"
  ON public.perfis_produtor FOR ALL USING (
    auth.uid() = (SELECT id FROM public.profiles WHERE id = profile_id)
  );

-- perfis_corporativa
CREATE POLICY "corporativa: leitura pública"
  ON public.perfis_corporativa FOR SELECT USING (true);
CREATE POLICY "corporativa: owner all"
  ON public.perfis_corporativa FOR ALL USING (
    auth.uid() = profile_id
  );

-- perfis_transportadora
CREATE POLICY "transportadora: leitura pública"
  ON public.perfis_transportadora FOR SELECT USING (true);
CREATE POLICY "transportadora: owner all"
  ON public.perfis_transportadora FOR ALL USING (
    auth.uid() = profile_id
  );

-- regioes_atendimento
CREATE POLICY "regioes: leitura pública"
  ON public.regioes_atendimento FOR SELECT USING (true);
CREATE POLICY "regioes: carrier owner"
  ON public.regioes_atendimento FOR ALL USING (
    auth.uid() = (SELECT profile_id FROM public.perfis_transportadora WHERE id = carrier_id)
  );

-- anuncios
CREATE POLICY "anuncios: leitura pública"
  ON public.anuncios FOR SELECT USING (status = 'ativo' OR seller_id = auth.uid());
CREATE POLICY "anuncios: seller insert"
  ON public.anuncios FOR INSERT WITH CHECK (auth.uid() = seller_id);
CREATE POLICY "anuncios: seller update"
  ON public.anuncios FOR UPDATE USING (auth.uid() = seller_id);
CREATE POLICY "anuncios: seller delete"
  ON public.anuncios FOR DELETE USING (auth.uid() = seller_id);

-- fotos_anuncio
CREATE POLICY "fotos: leitura pública"
  ON public.fotos_anuncio FOR SELECT USING (true);
CREATE POLICY "fotos: seller all"
  ON public.fotos_anuncio FOR ALL USING (
    auth.uid() = (SELECT seller_id FROM public.anuncios WHERE id = anuncio_id)
  );

-- rastreabilidade
CREATE POLICY "rastreabilidade: leitura pública"
  ON public.rastreabilidade FOR SELECT USING (true);
CREATE POLICY "rastreabilidade: seller all"
  ON public.rastreabilidade FOR ALL USING (
    auth.uid() = (SELECT seller_id FROM public.anuncios WHERE id = anuncio_id)
  );

-- favoritos
CREATE POLICY "favoritos: owner only"
  ON public.favoritos FOR ALL USING (auth.uid() = user_id);

-- propostas
CREATE POLICY "propostas: comprador ve as suas"
  ON public.propostas FOR SELECT USING (auth.uid() = buyer_id OR auth.uid() = seller_id);
CREATE POLICY "propostas: comprador insere"
  ON public.propostas FOR INSERT WITH CHECK (auth.uid() = buyer_id);
CREATE POLICY "propostas: vendedor responde"
  ON public.propostas FOR UPDATE USING (auth.uid() = seller_id OR auth.uid() = buyer_id);

-- contratos
CREATE POLICY "contratos: partes envolvidas"
  ON public.contratos FOR ALL USING (auth.uid() = buyer_id OR auth.uid() = seller_id);

-- solicitacoes_frete
CREATE POLICY "frete: requester ve as suas"
  ON public.solicitacoes_frete FOR SELECT USING (auth.uid() = requester_id);
CREATE POLICY "frete: transportadora ve abertas"
  ON public.solicitacoes_frete FOR SELECT USING (status = 'aberta');
CREATE POLICY "frete: requester all"
  ON public.solicitacoes_frete FOR INSERT WITH CHECK (auth.uid() = requester_id);
CREATE POLICY "frete: requester update"
  ON public.solicitacoes_frete FOR UPDATE USING (auth.uid() = requester_id);

-- cotacoes_frete
CREATE POLICY "cotacoes: carrier ve as suas"
  ON public.cotacoes_frete FOR SELECT USING (
    auth.uid() = (SELECT profile_id FROM public.perfis_transportadora WHERE id = carrier_id)
  );
CREATE POLICY "cotacoes: requester ve respostas"
  ON public.cotacoes_frete FOR SELECT USING (
    auth.uid() = (SELECT requester_id FROM public.solicitacoes_frete WHERE id = solicitacao_id)
  );
CREATE POLICY "cotacoes: carrier insere"
  ON public.cotacoes_frete FOR INSERT WITH CHECK (
    auth.uid() = (SELECT profile_id FROM public.perfis_transportadora WHERE id = carrier_id)
  );

-- mensagens
CREATE POLICY "mensagens: participantes"
  ON public.mensagens FOR ALL USING (
    auth.uid() = sender_id OR auth.uid() = receiver_id
  );

-- notificacoes
CREATE POLICY "notificacoes: owner only"
  ON public.notificacoes FOR ALL USING (auth.uid() = user_id);

-- ─────────────────────────────────────────────────────────────
-- 19. REALTIME — habilitar tabelas para subscriptions
-- ─────────────────────────────────────────────────────────────
